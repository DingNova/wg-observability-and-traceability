...
Status: Living working group document (v0.5-draft) - last revised 2026-09-19 (revision 5: multiple decisions per proposal - unresolved-applicability rule, supersession open choice, two fixtures)
...
- A denial and an approval recorded on the same proposal, with nothing relating them: both decisions stand; which one applies is reported as unresolved; the pair is not a conflict; if an execution is observed, enforcement is unknown.
